...
public abstract class AbstractCache<T> {
    ...
    // 引用计数, 除了普通的缓存功能，还需要维护一个计数
    // 除此之外，为了应对多进程的场景，还需要记录哪些资源正在从数据源中获取
    // (从数据源中获取资源是一个相对费事的操作)。于是有下面三个Map

    private HashMap<Long, T> cache;         // 实际缓存的数据

    private HashMap<Long, Integer> references;  // 元素的引用个数

    private HashMap<Long, Boolean> getting;     // 正在从数据源中获取的元素

    private int maxResource;                    // 缓存的最大资源数量
